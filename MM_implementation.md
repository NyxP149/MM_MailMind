...
La configuration Workbox exclut explicitement `/api` de la navigation de secours PWA. Les redirections OAuth et toutes les routes backend atteignent ainsi Nginx puis Express au lieu d’être remplacées par `index.html`, y compris lorsqu’un service worker contrôle déjà l’onglet.
